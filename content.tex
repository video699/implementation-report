...
\item<4-5>[\task{RETRIEVAL}] Find the \alert{nearest lecture materials} to a
  projection screen.
\item<5-5>[\task{MATCHING}] Decide if any nearest lecture materials
  \alert{matches} the projection screen.
\end{description}
\end{itemize}
...
\begin{frame}{System
  \titlenote{\autocites[sec.~4]{novotny18}{research-system}{implementation-system}}}
\begin{itemize}
\item<1-7> We built our system~\cite{implementation-system} in \alert{Python 3.4}
  using the following C++ libraries:
\begin{description}
\item<1-7>[\library{Annoy}] A library for \alert{approximate nearest neighbor
  retrieval}.
\item[\library{MuPDF}] An \alert{AGPL-licensed} library for \alert{rendering
  \abbr{PDF}, and \abbr{XPS} documents}.
\item<2-7>[\library{spatialindex}] A library for \alert{spatial indexing}.
\item<3-7>[\library{GEOS}] A library for \alert{computational geometry}.
\item<4-7>[\library{TensorFlow}] A library for \alert{building neural networks}.
\end{description}
\item<5-7> Here is how our system solves the individual \alert{subtasks}:
\begin{description}
\item<5-7>[\task{FRAMES}] We currently \alert{process all video frames}.
\item<6-7>[\task{SCREENS}] We \alert{use our dataset}~\cite{implementation-screens}
  to detect projection screens.  We implemented tracking of screen movements in
  \library{spatialindex}, and \library{GEOS}.
\item<7-7>[\task{RETRIEVAL}\bodytext{, }\task{MATCHING}] We implemented a supervised
