...
\item<1-5> Given a \alert{lecture recording} and a set of \alert{lecture materials},
  ...
  displays a lecture material. (\task{MAP})
...
\begin{itemize}
\item<1-8> We built our system~\cite{implementation-system} in \alert{Python 3.4}
  using the following C++ libraries:
\begin{description}
\item<1-8>[\library{Annoy}] A library for \alert{approximate nearest neighbor
  retrieval}.
\item<2-8>[\library{MuPDF}] An \alert{AGPL-licensed} library for \alert{rendering
  \abbr{PDF}, and \abbr{XPS} documents}.
\item<3-8>[\library{spatialindex}] A library for \alert{spatial indexing}.
\item<4-8>[\library{GEOS}] A library for \alert{computational geometry}.
\item<5-8>[\library{TensorFlow}] A library for \alert{building neural networks}.
\end{description}
\item<6-8> Here is how our system solves the individual \alert{subtasks}:
\begin{description}
\item<6-8>[\task{FRAMES}] We currently \alert{process all video frames}.
\item<7-8>[\task{SCREENS}] We \alert{use our dataset}~\cite{implementation-screens}
  to detect projection screens.  We implemented tracking of screen movements in
  \library{spatialindex}, and \library{GEOS}.
\item<8-8>[\task{RETRIEVAL}\bodytext{, }\task{CLASSIFICATION}] We implemented a supervised
